...
| 15 | 2026-09-25 | hook | 后台任务注入的提示也触发 `UserPromptSubmit`，把一轮的计时起点冲掉，长任务反被阈值拦掉 |
| 14 | 2026-09-23 | Telegram | 消息看不清别在 bot 侧折腾：Bot API 一个颜色字段都没有，气泡与引用块的底色全归客户端主题 |
...
- **Bot API 里没有任何样式参数。** 能发的实体只有粗体、斜体、下划线、删除线、剧透、等宽、引用、
  可折叠引用、链接、自定义 emoji —— 全部只说"这段是什么"，长什么样由客户端决定。
  引用块的灰底与左竖线、代码块的底色，都是主题按强调色画的，bot 碰不到。
- 试过并否决的版式（2026-09-23，发到真机逐条对比）：

  | 方案 | 结论 |
  |---|---|
  | 表头包进 `<blockquote>` | 是有一块实底，但引用块的底色同样是半透的，在花纹壁纸上没救回多少 |
  | 表头包进 `<pre>` | 块底最实，代价是等宽字体让中文变丑、**块内粗体失效**（`pre` 不允许套格式）、长行可能横向滚动 |
  | 色块 emoji 前缀 `🟩/🟨/🟥` | 没试。真正的问题是文字底下的花纹，不是标题不够显眼 |

- 解法：**客户端侧解决**，别在 bot 侧继续加壳。三选一 ——
  给这个聊天单独换纯色壁纸（进聊天 → ⋮ → 更改壁纸）、换一个气泡不透明的主题、
  或把 `.attheme` 导出来自己改（纯文本，一行一个 `键=颜色值`，可能附带壁纸图，改完导入即可）。
  本项目作者 2026-09-23 最终换了主题解决。
- 已回写代码：部分（`decorate` 与 `turn_activity` 的 `<code>`），其余写进 README 边界。

### 15. 一轮的计时起点被静默重置（2026-09-25，hook）
- 现象：跑了近 4 分钟的一轮结束了，手机上没有通知。日志写着 `skipped stop: turn 53s < 60s`。
- 现场（`remote_t2v` 的会话 `3408e93f`）：

  | 时刻 | 发生了什么 |
  |---|---|
  | 16:19:59 | 人发出「可以」，`UserPromptSubmit` 记下起点 |
  | 16:22:58 | 一个后台任务完成，往会话里注入 `<task-notification>` |
  | 16:23:51 | Stop 触发，算出 53 秒，被 `min_turn_seconds`（60）拦掉 |

  验算：`16:23:51 − 53s = 16:22:58`，与注入时刻差 0 秒。同一天另一条对得上第二次：
  `19:04:03 − 367s = 18:57:56`，也正是两条任务通知落地的时刻（那次侥幸过了 60 秒才发出来）。
- 原因：`UserPromptSubmit` 不只在人敲回车时触发。后台任务完成后注入会话的提示、排队发出的消息
  都会触发它，而 `handle_user_prompt` 当时无条件 `st["started_at"] = time.time()`。
  于是耗时只从最后一次注入算起 —— **越是会开后台 agent 的长任务，越容易被自己的阈值拦掉**。
- 解法：起点只记一次，`if not started: st["started_at"] = now`；由 Stop 负责清空。
  再加一道保险：起点超过 `STALE_TURN_SECONDS`（6 小时）视为上次 Stop 丢了（进程被杀、`/clear`），重新计时。
  同时给"保留起点"这条路径加了日志，这个事件原本什么都不写，出了问题只能靠时间戳反推。
- 已回写代码：是（`handlers.handle_user_prompt`），回归测试 `TurnStartTests`，其中
