...
- 0.4.0：消息形态 —— 「任务完成」默认 minimal（不带正文）+ 一行「这轮动了什么」，每条消息加分隔线与尾部空白。
- 0.4.1 / 0.4.2：文件名包 `<code>` 免得被当网址；一轮的计时起点只记一次，不被注入型提示冲掉。
- 0.5.0：人不在 Mac 前时 `min_turn_seconds` 不再适用（`away_after_seconds`）；新增排查速查表。
...
2. **常驻 bot 命令**：`/sessions`（列最近活跃会话）、`/mute 2h`、`/last`（重发上一条）。需要 launchd 守护进程，
   并把 `getUpdates` 消费者统一到守护进程、hook 改为向它询问决定。做了这一步，"多台机器共用 bot"也顺带解决。
3. **`setMyCommands` / `setMyDescription`**：给 bot 一个命令菜单和说明。只有 2 做了才有意义。
4. **Linux 空闲检测**：`xprintidle` 或 `loginctl`，让 `skip_if_mac_active_seconds` 在 Linux 上也生效。
5. **会话深链**：消息里放能直接打开该会话的 claude.ai 链接。目前 hook 输入里没有这个 URL，需先确认可推导。
