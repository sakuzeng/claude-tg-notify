...
**更新时间**：2026-09-25
**版本**：0.5.0（唯一事实源是 `claude_tg_notify/__init__.py` 的 `VERSION`，pyproject 动态读它）
...
| 人不在 Mac 前时短轮也推 | ✅ | 回放 09-25 21:05 的 54 秒现场：空闲 124s ≥ 120 → 发出，日志记下原因 |
| 注入型提示不再截断耗时 | ✅ | 用 09-25 的真实时序回放到已装的 hook：报 232 秒并发出，日志留下 keeping start |
